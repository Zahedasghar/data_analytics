...
g=ggplot()+geom_boxplot(data=df.exp,aes(x=year,y=NE.EXP.GNFS.CD, group=year))
...
df1.exp<-df.exp %>% filter(country=='Pakistan'|country=='Bangladesh')
ggplot(df1.exp,aes(x=year,y=NE.EXP.GNFS.CD,color=country))+geom_line()+labs(
