...
## VECM and Cointegration {background-color="`r clrs[1]`" .white}
Macro data are attached herewith. 

- Plot all the series
- Test for unit root (Use SBIC for lag selection)
- If all series are non-stationary, use Johansen Cointegration procedure for
case-3 option (constant in CE and in LR relation). Appropriate lag length selection is important.
...
library(gridExtra)
library(readr)
df<-read_excel("forecasting/docs/data/quarterly gdp 2021.xlsx")
  read_excel("docs/data/quarterly gdp 2021.xlsx")
df$quarter=as.yearqtr(df$date,format="%Yq%q")
...
```{r fig.align="center", echo = FALSE,fig.width =15,repr.plot.height=8}
df11<-df11 %>% filter(quarter>"1970 Q4" & quarter<="2019 Q1")
g1<-ggplot(df11)+aes(x=quarter,y=qrgdp)+geom_line()
g2<-ggplot(df11)+aes(x=quarter,y=lnM2)+geom_line()
g3<-ggplot(df11)+aes(x=quarter,y=CPI)+geom_line()
...
```{r fig.align="center", echo = FALSE,fig.width =14,repr.plot.height=6}
...
![](/Users/hp/Documents/money_price1.png) 

## $m_t$ is doing all the adjument 
![](/Users/hp/Documents/money_price2.png) 
...
## $m^*_t$ is doing all the adjument
 ![](/Users/hp/Documents/money_price3.png)
