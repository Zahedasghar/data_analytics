...
-   Understand important elements of data, summarizing, visualization
    and tabulating data as per best practices

-   Correlation and Regression, Linear and Nonlinear regressions

-   Forecasting education needs: enrollment, infrastructure, human
...
|           |                 | covered         | aterial         |   |
+-----------+-----------------+-----------------+-----------------+---+
| 1         | Int roduction   | In troduction   | [PO             |   |
|           | to software     | to R, RStudio,  | SIT](pos        |   |
|           |                 | POSIT Cloud ,   | it%20%20.cloud) |   |
|           |                 | gituhub         |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
|           | Data V          |                 | R for Data      |   |
|           | ocabulary,      |                 | Science         |   |
...
|           | R               |                 |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
| 2         | Data Vis        | bar, scatter,   | graphics for c  |   |
|           | ualization      | line and        | ategorical and  |   |
|           | (when to make   | histograms with | continuous data |   |
|           | which graph )   | e ducational    | using R,        |   |
|           |                 | case study data |                 |   |
|           |                 | in ggplot2      |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
|           | Lab on having   | In-class lab    | Ap plications   |   |
|           | data vis        |                 |                 |   |
|           | ualization      |                 |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
| 3         | Data Tables,    | Learning Data   | Tables from     |   |
|           | Measure of      | Tables with R   | various PIE re  |   |
|           | central         | as published in | sources and r   |   |
|           | tendency,       | world leading   | eproducing      |   |
|           | dispersion      | outlets         | tables like     |   |
|           |                 |                 | 538, NYT, WSJ,  |   |
|           |                 |                 | FT...           |   |
+-----------+-----------------+-----------------+-----------------+---+
|           | -   \*Lab\*\*   | Homework lab    |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
| 4         | -   E           | Data E          | dataxray in R   |   |
|           |     xploratory  | xploration not  |                 |   |
...
|           | minutes         |                 |                 |   |
+-----------+-----------------+-----------------+-----------------+---+
|           | Regression      | How to have     |                 |   |
|           | /Econ           | results of      |                 |   |
|           | ometric/Fo      | various models  |                 |   |
